...
class User {
    ...
    public function create($nama, $email, $alamat, $akre, $status, $visimisi, $foto)
    {
        $targetDir = "../user/";
        ...
        $sql = "INSERT INTO `tb_sekolah` (`id`, `nama`, `email`, `alamat`, `akre`, `status`, `visimisi`, `foto`) 
                VALUES (NULL, '$nama', '$email', '$alamat', '$akre', '$status', '$visimisi', '$foto_name')";
        $result = $this->conn->query($sql);
        ...
        header('Location: ../user/add-sekolah.php');
        exit();
    }

    public function search($keyword) {    
        $stmt = $this->conn->prepare("SELECT * FROM tb_sekolah WHERE nama LIKE ? OR status LIKE ? OR akre LIKE ?");
        $keyword = "%$keyword%";
    ...
    {
        $id = $_GET['id'];
        $stmt = $this->conn->prepare("SELECT * FROM tb_sekolah WHERE id=?");
    ...
    public function edit($id, $nama, $email, $alamat, $akre, $status, $visimisi, $foto)
    {
        if($foto && $foto["name"]) {
            $targetDir = "user/";
            $targetFile = $targetDir . basename($foto["name"]);      
        ...
        $stmt = $this->conn->prepare("UPDATE tb_sekolah SET nama=?, email=?, alamat=?, akre=?, status=?, visimisi=?, foto=? WHERE id=?");
        $stmt->bind_param("ssssssi", $nama, $email, $alamat, $akre, $status, $visimisi, $foto_name, $id);
